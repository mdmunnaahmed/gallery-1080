(function ($) {
	...
	$.fn.timePicky = function (options) {
		...
		return this.each(function () {
			...
			var $newElement = $('<div class="timepicker_wrap ' + settings.className + '">' + '<div class="arrow_top"></div>' + '<div class="time">' + topArrowButton + '<div class="ti_tx"><input type="text" class="timepicki-input"' + (settings.disableKeyboard ? "readonly" : "") + "></div>" + bottomArrowButton + "</div>" + '<div class="mins">' + topArrowButton + '<div class="mi_tx"><input type="text" class="timepicki-input"' + (settings.disableKeyboard ? "readonly" : "") + "></div>" + bottomArrowButton + "</div>");

			if (settings.showMeridian) {
				$newElement.append('<div class="meridian">' + topArrowButton + '<div class="mer_tx"><input type="text" class="timepicki-input" readonly></div>' + bottomArrowButton + "</div>");
			}
			...
			$(".timepicki-input").on("keydown", function (event) {
				...
				function validate() {
					...
					if ($grandfatherDiv.hasClass("time")) {
						if (isValidNumber) {
							var hours = settings.showMeridian ? Math.min(Math.max(parseInt(lastValue), 1), 12) : Math.min(Math.max(parseInt(lastValue), 0), 23);
